...
NETDATA.errorCodes = {
    ...
    407: {message: "无法对 HELLO netdata 服务器执行 HELLO 操作", alert: false},
    408: {message: "Netdata 服务器向 HELLO 发送了无效响应", alert: false},
    409: {message: "无法访问 netdata 注册表", alert: false},
    410: {message: "Netdata 注册表 ACCESS 失败", alert: false},
    411: {message: "Netdata 注册表服务器向 DELETE 发送无效响应", alert: false},
    412: {message: "Netdata 注册表 DELETE 失败", alert: false},
    413: {message: "Netdata 注册表服务器向 SWITCH 发送无效响应 ", alert: false},
    414: {message: "Netdata 注册表 SWITCH 失败", alert: false},
    415: {message: "Netdata 告警下载失败", alert: false},
    416: {message: "Netdata 告警日志下载失败", alert: false},
    417: {message: "Netdata 注册表服务器向 SEARCH 发送无效响应", alert: false},
    418: {message: "Netdata 注册表 SEARCH 失败", alert: false}
};
    ...
    object: function (name, obj, ignore_regex) {
        ...
        switch (typeof(obj)) {
            case 'string':
                ...
                if (ret !== obj) {
                    console.log('XSS 保护将字符串 ' + name + ' 从  "' + obj + '" 更改为 "' + ret + '"');
                }
                return ret;
            ...
            case 'object':
                ...
                if (Array.isArray(obj)) {
                    ...
                    for (var i in obj) {
                        ...
                        if (this.string(i) !== i) {
                            console.log('XSS 保护删除了无效的对象成员"' + name + '.' + i + '"');
                            delete obj[i];
                        } else {
...
NETDATA.options = {
    ...
    current: {
        ...
        // each 图表库 has its own limit too
        // the max of this and the 图表库 is used
...
NETDATA.localStorageGet = function (key, def, callback) {
    ...
    if (NETDATA.localStorageTest()) {
        try {
            ...
            console.log('localStorage：无法读取 “' + key.toString() + '”，使用默认值：“' + def.toString() + '”');
            ret = def;
        }
    }

    if (typeof ret === 'undefined' || ret === 'undefined') {
        console.log('localStorage： LOADED UNDEFINED “' + key.toString() + '” 作为值 ' + ret + ' 类型 ' + typeof(ret));
        ret = def;
    }
...
NETDATA.localStorageSet = function (key, value, callback) {
    if (typeof value === 'undefined' || value === 'undefined') {
        console.log('localStorage: ATTEMPT TO SET UNDEFINED "' + key.toString() + '" 作为值 ' + value + ' 类型 ' + typeof(value));
    }
    ...
    if (NETDATA.localStorageTest()) {
        ...
        try {
            ...
            console.log('localStorage: 无法保存  "' + key.toString() + '" 的值: "' + value.toString() + '"');
        }
    }
...
NETDATA.setOption = function (key, value) {
    if (key.toString() === 'setOptionCallback') {
        ...
        if (typeof NETDATA.localStorage.default[name.toString()] === 'undefined') {
            console.log('localStorage: setOption()  在未保存的选项上: "' + name.toString() + '", 值: ' + value);
        }
            ...
            y: {
                ...
                axisLabelFormatter: function (y) {
                    ...
                    if (state.units_desired === 'auto' && typeof old_units !== 'undefined' && new_units !== old_units && !NETDATA.chartLibraries.dygraph.isSparkline(state)) {
                        ...
                        if (len < 0) {
                            state.log('单位差异，但找不到要更改的 dygraphs div: old = ' + old_units + ', new = ' + new_units);
                        }
                    }
...
NETDATA.d3pieClearSelection = function (state, force) {
    ...
    if (state.isAutoRefreshable() && state.data !== null && force !== true) {
        ...
        if (state.tmp.d3pie_last_slot !== -1) {
            ...
            NETDATA.d3pieChange(state, [{label: 'no data', value: 1, color: '#666666'}], '无可用数据');
        }
    }
...
NETDATA.registerChartLibrary = function (library, url) {
    if (NETDATA.options.debug.libraries) {
        console.log("registering 图表库: " + library);
    }
...
let chartState = function (element) {
    ...
    if (typeof this.id === 'undefined') {
        error("netdata 元素需要 data-netdata");
        return;
    }
    ...
    const lateInitialization = () => {
        ...
        if (typeof this.force_update_every !== 'number' || this.force_update_every <= 1) {
            if (this.force_update_every !== null) {
                this.log('忽略属性 data-update-every 的无效值');
            }
        ...
        const getForeignElementById = (opt) => {
            ...
            if (typeof el === 'undefined') {
                this.log('找不到具有 name 的元素"' + id.toString() + '"');
                return null;
            }
    ...
    const callChartLibraryUpdateSafely = (data) => {
        ...
        if (!status) {
            error('图表无法更新为 ' + this.library_name);
            return false;
        }
    ...
    const callChartLibraryCreateSafely = (data) => {
        ...
        if (!status) {
            error('图表创建失败为 ' + this.library_name);
            return false;
        }
    ...
    // resizeChart() - private
    // to be called just before the 图表库 to make sure that
    ...
    const resizeChart = () => {
        if (this.tm.last_resized < NETDATA.options.last_page_resize) {
            ...
            if (this.needsRecreation()) {
                if (this.debug) {
                    this.log('resizeChart(): 初始化图表');
                }

                init('force');
            } else if (typeof this.library.resize === 'function') {
                if (this.debug) {
                    this.log('resizeChart(): 调整大小图表');
                }
    ...
    this.__chartDimensionColor = function (label) {
        ...
        // for this chart only, since the 图表库
    ...
    this.legendUpdateDOM = function () {
        ...
        if (this.hasLegend()) {
            ...
            if (NETDATA.options.current.legend_toolbox && this.library.toolboxPanAndZoom !== null) {
                ...
                if (NETDATA.options.current.show_help) {
                    $(this.element_legend_childs.toolbox_left).popover({
                        ...
                        title: 'Pan Left',
                        content: '将图表向左平移。您也可以用鼠标或手指<b>拖动它</b>（在触摸设备上）。<br/><small>可以从设置中禁用帮助。</small>'
                    });
                }
                ...
                if (NETDATA.options.current.show_help) {
                    $(this.element_legend_childs.toolbox_reset).popover({
                        ...
                        title: 'Chart Reset',
                        content: '将所有图表重置为默认的自动刷新状态。您也可以用鼠标或手指<b>双击</b>图表内容（在触摸设备上）。<br/><small>可以从设置中禁用帮助。</small>'
                    });
                }
                ...
                if (NETDATA.options.current.show_help) {
                    $(this.element_legend_childs.toolbox_right).popover({
                        ...
                        title: 'Pan Right',
                        content: '将图表向右平移。您也可以用鼠标或手指<b>拖动它</b>（在触摸设备上）。<br/><small>帮助，可以从设置中禁用。</small>'
                    });
                }
                ...
                if (NETDATA.options.current.show_help) {
                    $(this.element_legend_childs.toolbox_zoomin).popover({
                        ...
                        title: 'Chart Zoom In',
                        content: '放大图表。您也可以按 Shift 键并选择图表的某个区域，或者按 Shift 键或 Alt 键并使用鼠标滚轮或双指触摸板滚动来放大或缩小。<br/><small>帮助，可以从设置中禁用。</small>'
                    });
                }
                ...
                if (NETDATA.options.current.show_help) {
                    $(this.element_legend_childs.toolbox_zoomout).popover({
                        ...
                        content: '缩小图表。您也可以按 SHIFT 或 ALT 并使用鼠标滚轮或 2 指触摸板滚动来放大或缩小。<br/><small>帮助，可以从设置中禁用。</small>'
                    });
                }
            ...
            if (NETDATA.options.current.resize_charts) {
                ...
                if (NETDATA.options.current.show_help) {
                    $(this.element_legend_childs.resize_handler).popover({
                        ...
                        title: 'Chart Resize',
                        content: '用鼠标或手指（在触摸设备上）拖动此点，以垂直调整图表大小。您也可以双击它或<b>双击它</b>以<b></b>在 2 种状态之间重置：默认状态和适合所有值的状态。<br/><small>帮助，可以从设置中禁用。</small>'
                    });
                }
            ...
            if (NETDATA.options.current.show_help) {
                $(content).popover({
                    ...
                    content: '您可以单击或点击值或标签来选择尺寸。通过按 SHIFT 或 CONTROL，您可以启用或禁用多个维度。<br/><small>帮助，可以从设置中禁用。</small>'
                });
            }
        } else {
            this.element_legend_childs = {
        ...
        // the original legend of the 图表库
    ...
    this.updateChart = function (callback) {
        ...
        if (!this.library.initialized) {
            if (this.library.enabled) {
                if (this.debug) {
                    this.log('updateChart(): initializing 图表库');
                }
                ...
                error('图表库 "' + this.library_name + '" is not available.');
            ...
            .fail(function (msg) {
                ...
                if (msg.statusText !== 'abort') {
                    ...
                    if (that.retries_on_data_failures > NETDATA.options.current.retries_on_data_failures) {
                        ...
                        that.retries_on_data_failures = 0;
                        error('URL 的数据下载失败： ' + that.data_url);
                    }
                    else {
                        that.tm.last_autorefreshed = Date.now();
    ...
    this.canBeAutoRefreshed = function () {
        if (!this.enabled) {
            if (this.debug) {
                this.log('canBeAutoRefreshed() -> 未启用');
            }
        ...
        if (this.running) {
            if (this.debug) {
                this.log('canBeAutoRefreshed() -> 已在运行');
            }
        ...
        if (this.library === null || this.library.enabled === false) {
            error('图表库 "' + this.library_name + '" 不可用');
            if (this.debug) {
                this.log('canBeAutoRefreshed() -> 图表库 ' + this.library_name + ' 不可用');
            }
        ...
        if (!this.isVisible()) {
            if (NETDATA.options.debug.visibility || this.debug) {
                this.log('canBeAutoRefreshed() -> 不可见');
            }
        ...
        if (this.current.force_update_at !== 0 && this.current.force_update_at < now) {
            if (this.debug) {
                this.log('canBeAutoRefreshed() -> 定时强制更新 - 允许此更新');
            }
        ...
        if (!this.isAutoRefreshable()) {
            if (this.debug) {
                this.log('canBeAutoRefreshed() -> 不可自动刷新');
            }
        ...
        if (NETDATA.options.page_is_visible === false && this.updates_counter && this.updates_since_last_unhide) {
            if (NETDATA.options.debug.focus || this.debug) {
                this.log('canBeAutoRefreshed() -> 不是第一次更新，并且页面没有焦点');
            }
        ...
        if (this.needsRecreation()) {
            if (this.debug) {
                this.log('canBeAutoRefreshed() -> 需要重新创造.');
            }
        ...
        if (NETDATA.options.auto_refresher_stop_until >= now) {
            if (this.debug) {
                this.log('canBeAutoRefreshed() -> 停止直到将来。');
            }
        ...
        if (NETDATA.globalPanAndZoom.isActive()) {
            if (NETDATA.globalPanAndZoom.shouldBeAutoRefreshed(this)) {
                if (this.debug) {
                    this.log('canBeAutoRefreshed(): 全局平移：我需要更新。');
                }
                ...
                if (this.debug) {
                    this.log('canBeAutoRefreshed(): 全局平移：我已经是最新的了。');
                }
        ...
        if (this.selected) {
            if (this.debug) {
                this.log('canBeAutoRefreshed(): 我有一个选择。');
            }
        ...
        if (this.paused) {
            if (this.debug) {
                this.log('canBeAutoRefreshed(): 我停顿了一下。');
            }
        ...
        if (now - this.tm.last_autorefreshed >= data_update_every) {
            if (this.debug) {
                this.log('canBeAutoRefreshed(): 是时候更新我了。现在: ' + now.toString() + ', last_autorefreshed: ' + this.tm.last_autorefreshed + ', data_update_every: ' + data_update_every + ', delta: ' + (now - this.tm.last_autorefreshed).toString());
            }
...
NETDATA.parseDom = function (callback) {
    ...
    if (NETDATA.options.debug.main_loop) {
        console.log('DOM updated - 页面上有 ' + targets.length + ' 图表。');
    }
...
NETDATA.alarms = {
    ...
    notify: function (entry) {
        ...
        switch (entry.status) {
            ...
            default:
                console.log('无效的警报状态 ' + entry.status);
                return;
        }
...
NETDATA.loadRequiredJs = function (index, callback) {
    ...
    $.ajax({
        ...
        .fail(function () {
            alert('无法加载所需的 JS 库: ' + NETDATA.requiredJs[index].url);
        })
        .always(function () {
            NETDATA.loadedRequiredJs++;
