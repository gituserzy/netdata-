        ...
        while (len--) {
            if (typeof urlOptions[numeric[len]] === 'string') {
                try {
                    ...
                    console.log('无法解析 URL 哈希参数 ' + numeric[len]);
...
function renderMachines(machinesArray) {
    ...
    if (machinesArray === null) {
        ...
        if (ret) {
            ...
            console.log("无法联系注册表 - 从浏览器本地存储加载注册表数据");
        }
    }
...
function errorMyNetdataMenu() {
    setMyNetdataMenu(`<div class="agent-item" style="white-space: nowrap">
        ...
        无法从 netdata.cloud 加载已知的 netdata 代理！
        <div></div>
    </div>`);
}
...
function renderMyNetdataMenu(machinesArray) {
    ...
    if (machinesArray == registryAgents) {
        ...
        console.log("从 netdata.cloud 呈现 my-netdata 菜单", machinesArray);
    }
    ...
    if (!isSignedIn()) {
        html += (
            ...
            <div class="agent-item">
                ...
                <a href="#" onclick="switchRegistryModalHandler(); return false;">交换机身份</a>
                <div></div>
            </div>
            <div class="agent-item">
        ...
        html += (
            ...
            <div class="agent-item">
                ...
                <a href="#" onclick="showSyncModal(); return false">与 netdata.cloud 同步</a>
                <div></div>
            </div>
            <div class="agent-item">
                <i class="fas fa-question-circle""></i>
            ...
            if (typeof data !== 'undefined' && data !== null && typeof data.machine_guid === 'string' && data.machine_guid === guid) {
                ...
                if (!gotoServerStop) {
                    ...
                    if (gotoServerMiddleClick) {
                        ...
                        document.getElementById('gotoServerResponse').innerHTML = '<b>打开新窗口 ' + NETDATA.registry.machines[guid].name + '<br/><a href="' + verifyURL(finalURL) + '">' + escapeUserInputHTML(url) + '</a></b><br/>(检查您的弹出窗口阻止程序是否失败)';
                    } else {
                ...
                if (gotoServerValidateRemaining <= 0) {
                    gotoServerMiddleClick = false;
                    document.getElementById('gotoServerResponse').innerHTML = '<b>不好意思！我找不到此服务器的任何操作 URL</b>';
                }
            }
        });
    }, (id * 50) + penalty);
    ...
    if (!isSignedIn()) {
        ...
        setTimeout(function () {
            if (gotoServerStop === false) {
                document.getElementById('gotoServerResponse').innerHTML = '<b>添加了此计算机的所有已知 URL。</b>';
    ...
    if (n !== '' && n.length === 36) {
        NETDATA.registry.switch(n, function (result) {
            if (result !== null) {
                ...
                document.getElementById('switchRegistryResponse').innerHTML = "<b>不好意思！注册局拒绝了您的请求。</b>";
            }
        });
    } else {
        document.getElementById('switchRegistryResponse').innerHTML = "<b>您输入的 ID 不是 GUID。</b>";
    }
}
    ...
    if (deleteRegistryUrl) {
        if (isSignedIn()) {
            deleteCloudAgentURL(deleteRegistryGuid, deleteRegistryUrl)
                .then((count) => {
                    if (!count) {
                        responseEl.innerHTML = "<b>对不起，此命令被 netdata.cloud 拒绝了！</b>";
                        return;
                    }
            ...
            NETDATA.registry.delete(deleteRegistryUrl, function (result) {
                if (result !== null) {
                    ...
                    responseEl.innerHTML = "<b>对不起，此命令被注册表服务器拒绝！</b>";
                }
            });              
        }
    }
...
function renderPage(menus, data) {
    ...
    sidebar += '<li class="" style="margin:20px;color:#666;"><small>netdata 运作于 <b>' + data.hostname.toString() + '</b>, 每 ' + ((data.update_every === 1) ? '秒' : data.update_every.toString() + ' 秒') + ' 搜集 <b>' + data.dimensions_count.toLocaleString() + '</b> 项指标，提供 <b>' + data.charts_count.toLocaleString() + '</b> 张图表以及监视 <b>' + data.alarms_count.toLocaleString() + '</b> 个警报，使用 ' + Math.round(data.rrd_memory_bytes / 1024 / 1024).toLocaleString() + ' MB 内存储存 ' + NETDATA.seconds4human(data.update_every * data.history, {space: '&nbsp;'}) + ' 的即时资料。<br/>&nbsp;<br/><b>netdata</b><br/>' + data.version.toString() + '</small></li>';
    sidebar += '</ul>';
    div.innerHTML = html;
    ...
    NETDATA.alarms.get('all', function (data) {
        ...
        function alarm_lookup_explain(alarm, chart) {
            ...
            if (typeof alarm.lookup_dimensions !== 'undefined') {
                ...
                var x = d.split(',');
                if (x.length > 1) {
                    dimensions = '维度之和 <code>' + alarm.lookup_dimensions + '</code> ';
                } else {
                    dimensions = '所有维度值 <code>' + alarm.lookup_dimensions + '</code> ';
                }
            }

            return '图表 <code>' + alarm.chart + '</code>的<code>' + alarm.lookup_method + '</code> '
                + dimensions + ','
                + ', 从 <code>' + NETDATA.seconds4human(alarm.lookup_after + alarm.lookup_before, {space: '&nbsp;'}) + '</code>开始， 一直到 <code>' + NETDATA.seconds4human(alarm.lookup_before, {space: '&nbsp;'}) + '</code>'
                + ((alarm.lookup_options) ? (', 带有选项 <code>' + alarm.lookup_options.replace(/ /g, ',&nbsp;') + '</code>') : '')
                + '.';
        }
        ...
        function alarm_to_html(alarm, full) {
            ...
            if (typeof (chart) === 'undefined') {
                ...
                if (typeof (chart) === 'undefined') {
                    ...
                    // are now available.
                    console.log('找不到图表 ' + alarm.chart + ', 您可能需要刷新页面.');
                    return '';
                }
            }
            ...
            var action_buttons = '<br/>&nbsp;<br/>role: <b>' + alarm.recipient + '</b><br/>&nbsp;<br/>'
                + '<div class="action-button ripple" title="单击以将仪表板滚动到此警报的图表" data-toggle="tooltip" data-placement="bottom" onClick="scrollToChartAfterHidingModal(\'' + alarm.chart + '\'); $(\'#alarmsModal\').modal(\'hide\'); return false;"><i class="fab fa-periscope"></i></div>'
                + '<div class="action-button ripple" title="单击以将此徽章的 URL 复制到剪贴板" data-toggle="tooltip" data-placement="bottom" onClick="clipboardCopy(\'' + badge_url + '\'); return false;"><i class="far fa-copy"></i></div>'
                + '<div class="action-button ripple" title="单击以将此锁屏提醒的自动刷新<code>嵌入</code> HTML 元素复制到剪贴板" data-toggle="tooltip" data-placement="bottom" onClick="clipboardCopyBadgeEmbed(\'' + badge_url + '\'); return false;"><i class="fas fa-copy"></i></div>';
        ...
        for (x in data.alarms) {
            ...
            if (typeof chart === 'undefined') {
                console.log('WARNING: alarm ' + x + ' is linked to chart ' + alarm.chart + ', 在从服务器获取的图表列表中找不到。');
        ...
        if (!count_active) {
            active += '<div style="width:100%; height: 100px; text-align: center;"><span style="font-size: 50px;"><i class="fas fa-thumbs-up"></i></span><br/>一切都很正常。没有发出警报。</div>';
        } else {
            active += footer;
        }

        if (!count_all) {
            all += "<h4>此系统中未运行任何警报。</h4>";
        } else {
            all += footer;
        }
            ...
            $('#alarms_log_table').bootstrapTable({
                ...
                columns: [
                    ...
                    },
                    {
                        field: 'duration',
                        title: 'Last Duration',
                        titleTooltip: '警报处于其先前状态的持续时间，就在此事件之前',
                        formatter: function (value, row, index) {
                            void (row);
                            void (index);
                    ...
                    },
                    {
                        ...
                        titleTooltip: '在此事件发生之前发出警报的持续时间',
                        formatter: function (value, row, index) {
                            void (row);
                            void (index);
                            return NETDATA.seconds4human(value, {negative_suffix: '', space: ' ', now: 'no time'});
                    ...
                    },
                    {
                        field: 'updated',
                        title: '更新状态',
                        titleTooltip: '当此事件已被另一个事件更新时为 True',
                        formatter: function (value, row, index) {
                            void (row);
                            void (index);
                    ...
                    },
                    {
                        field: 'updates_id',
                        title: 'Updates ID',
                        titleTooltip: '由于此事件而过时的事件的唯一 ID',
                        formatter: function (value, row, index) {
                            void (row);
                            void (index);
                            return alarmid4human(value);
                    ...
                    },
                    {
                        ...
                        formatter: function (value, row, index) {
                            ...
                            if (value === 0) {
                                ...
                                return 'FAILED（使用代码' + value.toString() + ')';
                            }
                        },
                        align: 'center',
                        valign: 'middle',
...
function notifyForUpdate(force) {
    ...
    checkForUpdateByVersion(force, function (sha1, sha2) {
        ...
        if (sha1 === null) {
            save = false;
            versionLog('<p><big>取得您的 netdata 版本失败！</big></p><p>您始终可以从 <a href=“https://github.com/netdata/netdata” target=“_blank”>its github 页面获取最新的网络数据</a>。</p>');
        } else if (sha2 === null) {
            save = false;
            ...
            versionLog('<p><big><strong>netdata的新版本可用！</strong></big></p><p>最新版本： <b><code>' + sha2 + '</code></b></p><p><a href="' + compare + '" target="_blank">单击此处查看更改日志</a> 且<br/><a href="https://github.com/netdata/netdata/tree/master/packaging/installer/UPDATE.md" target="_blank">单击此处获取有关更新</a> NetData 安装的说明。</p><p>我们建议您查看更改日志，了解您可能感兴趣的新功能或您可能需要的重要错误修复。<br/>保持网络数据更新通常是一个好主意。</p>');
...
function loadSnapshot() {
    ...
    if (tmpSnapshotData === null) {
        ...
        loadSnapshotModalLog('danger', '未加载任何数据');
        return;
    }
...
function loadSnapshotPreflightFile(file) {
    ...
    fr.onload = function (e) {
        ...
        try {
            ...
            loadSnapshotModalLog('success', '文件加载后，点击<b>导入</b>进行渲染！');
...
function saveSnapshotModalShowExpectedSize() {
    ...
    saveSnapshotModalLog(priority, '快照每个维度将有 ' + points.toString() + ' 点. 磁盘上的预期大小 ' + sizemb + ' MB, 在浏览器内存中 ' + memmb + ' MB.<br/>' + msg);
}
...
function saveSnapshot() {
    loadPako(function () {
        loadLzString(function () {
            ...
            saveSnapshotModalLog('info', '生成快照 ' + saveData.data_points.toString() + ' data points per dimension...');
                ...
                setTimeout(function () {
                    ...
                    state.updateChart(function (status, reason) {
                        ...
                        if (status === true) {
                            ...
                            charts_failed++;
                            state.log('更新失败: ' + reason);
                        }

                        saveSnapshotModalLog((charts_failed) ? 'danger' : 'info', '生成的快照数据大小<b>' + (Math.round(size * 100 / 1024 / 1024) / 100).toString() + ' MB</b>. ' + ((charts_failed) ? (charts_failed.toString() + ' charts have failed to be downloaded') : '').toString() + info);
...
function dashboardSettingsSetup() {
    var update_options_modal = function () {
        ...
        var timezone_sync_option = function (option) {
            ...
            document.getElementById('current_timezone').innerText = (NETDATA.options.current.timezone === 'default') ? 'unset, 使用浏览器默认值' : NETDATA.options.current.timezone;
...
function runOnceOnDashboardWithjQuery() {
    ...
    $.fn.shorten = function (settings) {
        ...
        var config = {
            ...
            minHideChars: 10,
            ellipsesText: "...",
            moreText: '<i class="fas fa-expand"></i> 显示更多信息',
            lessText: '<i class="fas fa-compress"></i> 显示更少信息',
            onLess: function () {
                NETDATA.onscroll();
            },
            onMore: function () {
                        ...
                        if (content[i] === '<' && !inTag) {
                            ...
                            if (tagName[0] === '/') {
                                ...
                                if (tagName !== ('/' + openTags[0])) {
                                    config.errMsg = '错误 en HTML： 堆栈的顶部应该是关闭的标签';
                                } else {
...
var selected_server_timezone = function (timezone, status) {
    ...
    if (typeof status === 'undefined') {
        ...
        if (NETDATA.dateTime.init(timezone) === false) {
            ...
            document.getElementById('timezone_error_message').innerHTML = '哎呀！您的浏览器不接受该时区。请打开 github 问题来帮助我们修复它。';
        ...
        if (NETDATA.dateTime.init(timezone) === false) {
            ...
            document.getElementById('timezone_error_message').innerHTML = '不好意思。您的浏览器不接受时区 “' + timezone.toString() + '”。请从列表中选择一个。';
            NETDATA.setOption('user_set_server_timezone', NETDATA.options.server_timezone);
        }
    }
    ...
    if (isSignedIn()) {
        ...
        container.innerHTML = (
            `<a href="#" class="btn" onclick="signInDidClick(event); return false">
                <i class="fas fa-sign-in-alt"></i>&nbsp;<span class="hidden-sm hidden-md">登入</span>
            </a>`
        )
    }
}
...
function syncAgents(callback) {
    ...
    console.log("检查是否需要同步。", localAgents);
    ...
    if (agentsToSync) {
        console.log("与 netdata.cloud 同步。");
